...
long initapp (long argc, char **argv)
{
	...
	*/
	...
	/**
	 * Getkv6 loads the voxel object (spade.kv6) into memory and returns a
	 ...
	 */
	spade.voxnum = getkv6("kv6\\spade.kv6");

	/**
	 * Place the spades's position at a spot in the middle of the VXL map. Usually,
	 * you would load the sprite's location from the SXL file. This is just for
	 * demonstration.
	 */
	spade.p.x = 256; spade.p.y = 256; spade.p.z = 45.5;

	/**
	 * Select the spades's orientation. I use an identity matrix here.
	 */
	spade.s.x = 8;  spade.s.y = 0;   spade.s.z = 0;
	spade.h.x = 0;   spade.h.y = 8;  spade.h.z = 0;
	spade.f.x = 0;   spade.f.y = 0;   spade.f.z = 8;
	 ...
	 */
	spade.kfatim = 0; spade.okfatim = 0; spade.flags = 1;
...
void doframe ()
{
	...
	/**
	 * Renders the voxel sprite, using the data from the spade structure.
 ...
 */
void uninitapp () { freekv6(spade.voxnum); /*uninitvoxlap();*/ kzuninit(); }
