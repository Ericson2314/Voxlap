...
#define MAXZSIZ 1024                //Code taken from renderboundcube of SLAB6D (Pentium III version :)

// Related to Updates Lighting, Mip-mapping, and Floating objects list
...
static void kv6draw (vx5sprite *spr)
{
...
#if (USEZBUFFER == 0)
	lpoint3d lp;
	/** @warning Very crappy Z-buffer! */
	if (!cansee(&gipos,&spr->p,&lp)) return;
#endif
    ...
    /** Find split point by using  Cramer's rule @link http://en.wikipedia.org/wiki/Cramer%27s_rule
	...
	if (kv->zsiz >= MAXZSIZ) return; /** @todo HACK TO PREVENT CRASHES FOR NOW... FIX! */
	...
	if (cputype&(1<<25))
	{
		...
		#elif defined(_MSC_VER) //msvc inline asm
		_asm
		{
			movq	mm6, qsum0
			movq	mm7, qsum1
